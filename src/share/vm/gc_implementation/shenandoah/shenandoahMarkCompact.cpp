...
void ShenandoahMarkCompact::distribute_slices(ShenandoahHeapRegionSet** worker_slices) {
  ...
  for (size_t wid = 0; wid < n_workers; wid++) {
    ...
    while (r != NULL) {
      ...
      assert(ShenandoahPrepareForCompactionTask::is_candidate_region(r), err_msg("Sanity: " SIZE_FORMAT, num));
      assert(!map.at(num), err_msg("No region distributed twice: " SIZE_FORMAT, num));
      map.at_put(num, true);
      r = it.next();
    }
  }
...
void ShenandoahMarkCompact::compact_humongous_objects() {
  ...
  for (size_t c = heap->num_regions(); c > 0; c--) {
    ...
    if (r->is_humongous_start()) {
      ...
      assert(r->is_stw_move_allowed(), err_msg("Region " SIZE_FORMAT " should be movable", r->region_number()));
