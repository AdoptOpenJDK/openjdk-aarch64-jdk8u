...
#define GC_PAR_PHASE_DECLARE_WORKER_DATA(type, title) \
  _gc_par_phases[ShenandoahPhaseTimings::type] = new ShenandoahWorkerDataArray<double>(max_workers, title);
...
void ShenandoahPhaseTimings::record_workers_end(Phase phase) {
  ...
  assert(phase == init_evac ||
         ...
         err_msg("Phase should accept accept per-thread phase times: %s", phase_name(phase)));
