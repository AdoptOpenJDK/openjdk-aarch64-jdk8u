...
ShenandoahPhaseTimings::Phase ShenandoahPhaseTimings::worker_par_phase(Phase phase, GCParPhases par_phase) {
  assert(is_worker_phase(phase), err_msg("Phase should accept worker phase times: %s", phase_name(phase)));
  Phase p = Phase(phase + 1 + par_phase);
  assert(p >= 0 && p < _num_phases, err_msg("Out of bound for: %s", phase_name(phase)));
  return p;
}
...
ShenandoahWorkerData* ShenandoahPhaseTimings::worker_data(Phase phase, GCParPhases par_phase) {
  ...
  assert(wd != NULL, err_msg("Counter initialized: %s", phase_name(p)));
  return wd;
}
...
void ShenandoahPhaseTimings::set_cycle_data(Phase phase, double time) {
  ...
  assert(d == 0, err_msg("Should not be set yet: %s, current value: %lf", phase_name(phase), d));
#endif
...
void ShenandoahPhaseTimings::record_workers_start(Phase phase) {
  ...
  assert(_current_worker_phase == _invalid_phase,
          err_msg("Should not be set yet: requested %s, existing %s", phase_name(phase), phase_name(_current_worker_phase)));
...
ShenandoahWorkerTimingsTracker::ShenandoahWorkerTimingsTracker(ShenandoahPhaseTimings::GCParPhases par_phase, uint worker_id) :
  ...
  assert(_timings->worker_data(_phase, _par_phase)->get(_worker_id) == ShenandoahWorkerData::uninitialized(),
         err_msg("Should not be set yet: %s", ShenandoahPhaseTimings::phase_name(_timings->worker_par_phase(_phase, _par_phase))));
